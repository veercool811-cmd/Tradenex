...
app.delete(
  ...
  async (req, res) => {
    try {
      ...
      /*
        Find all deposits belonging to the deleted user.
        These deposit IDs are also used by referral commission
        transactions, so they must be removed together.
      */
      const deletedDepositIds = new Set(
        deposits
          .filter(
            (d) => String(d.userId || "") === deletedUserId
          )
          .map((d) => String(d.id))
      );

      /*
        Reverse referral commissions generated by this user's
        deposits before deleting the user.
      */
        ...
        transactions.filter(
          (t) =>
            ...
            (
              ...
              deletedDepositIds.has(String(t.depositId || ""))
            )
        );
      ...
      /*
        Remove the deleted user from every referrer's
        referral list.
      */
      for (const referrer of users) {
        if (!Array.isArray(referrer.referrals)) continue;
      ...
      */
      ...
      /*
        Remove all financial records belonging to the
        deleted user.
      */
      const cleanedDeposits =
        deposits.filter(
          (d) =>
            String(d.userId || "") !== deletedUserId
        );

      const cleanedWithdrawals =
        withdrawals.filter(
          (w) =>
            String(w.userId || "") !== deletedUserId
        );

      /*
        Remove:
        1. Normal transactions of deleted user
        2. Deposit transactions belonging to deleted deposits
        3. Referral commissions generated by deleted user's deposits
      */
      const cleanedTransactions =
        transactions.filter(
          (t) => {
            if (
              String(t.userId || "") === deletedUserId
            ) {
              return false;
            }
      ...
      /*
        Wait for previous PostgreSQL writes before deleting
      ...
      */
      ...
      /*
        Update in-memory cache and local JSON files.
      */
      ...
      */
      write(USERS_FILE, users);
      write(DEPOSITS_FILE, cleanedDeposits);
      write(WITHDRAWALS_FILE, cleanedWithdrawals);
      write(TRANSACTIONS_FILE, cleanedTransactions);
      ...
      res.json({
        success: true,
        message:
          "User, deposits, withdrawals and transactions deleted successfully.",
        user: publicUser(deletedUser),
        deleted: {
          deposits: deposits.length - cleanedDeposits.length,
          withdrawals:
            withdrawals.length - cleanedWithdrawals.length,
          transactions:
            transactions.length - cleanedTransactions.length,
        },
      });
    } catch (error) {
      console.error(
...
/* =====================================================
   ...
   Removes the deposit and reverses its financial effects.
===================================================== */
...
app.delete(
  ...
  async (req, res) => {
    try {
      ...
      /*
        Reverse only an approved deposit.
        Pending/Rejected deposits have not increased balance.
      */
      if (deposit.status === "Approved") {
        const user = users.find(
          (u) => String(u.id) === String(deposit.userId)
        );
        ...
        if (user) {
          ...
          user.totalDeposit = Math.max(
            0,
            number(user.totalDeposit) - amount
          );
        }

        /*
          Reverse referral commissions generated by this deposit.
        */
        const referralTransactions =
          transactions.filter(
            (t) =>
              t.source === "referralCommission" &&
              String(t.depositId || "") === String(deposit.id)
          );
      ...
      /*
        Remove the deposit transaction and its referral
        commission transactions.
      */
      const cleanedTransactions =
        transactions.filter(
          (t) =>
            String(t.referenceId || "") !== String(deposit.id) &&
            !(
              t.source === "referralCommission" &&
              String(t.depositId || "") === String(deposit.id)
            )
      ...
      res.json({
        success: true,
        message: "Deposit deleted and financial effects reversed.",
        deposit,
      });
    } catch (error) {
      console.error(
        "ADMIN DEPOSIT DELETE ERROR:",
        error
      );
